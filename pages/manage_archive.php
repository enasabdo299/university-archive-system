...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($target_id > 0) {
        try {
            if ($action === 'approve') {
                $stmt = $pdo->prepare("UPDATE users SET status = 'approved' WHERE id = ? AND role = 'archive'");
                $stmt->execute([$target_id]);
                $action_msg  = 'تم قبول الموظف بنجاح.';
                $action_type = 'success';
            } elseif ($action === 'reject') {
                $stmt = $pdo->prepare("UPDATE users SET status = 'rejected' WHERE id = ? AND role = 'archive'");
                $stmt->execute([$target_id]);
                $action_msg  = 'تم رفض الموظف بنجاح.';
                $action_type = 'warning';
            } elseif ($action === 'delete') {
                $stmt = $pdo->prepare("DELETE FROM users WHERE id = ? AND role = 'archive'");
                $stmt->execute([$target_id]);
                $action_msg  = 'تم حذف الموظف بنجاح.';
                $action_type = 'danger';
            }
            ...
            <div class="students-table-wrap">
                <div class="table-header" style="flex-wrap: wrap; gap: 15px;">
                    <div style="display: flex; align-items: center; gap: 10px;">
                        <h3><i class="fas fa-list"></i> قائمة موظفي الأرشيف</h3>
                    ...
                    <table class="students-table">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    <?php
                                    $st = $student['status'] ?? 'approved';
                                    $st_labels = ['approved' => 'نشط', 'rejected' => 'موقوف'];
                                ...
                                <td>
                                    <div class="actions-cell">
                                        ...
                                        <form method="POST" style="display:inline;">
                                            ...
                                            <button type="submit" class="act-btn approve" title="تنشيط">
                                        ...
                                        <form method="POST" style="display:inline;">
                                            ...
                                            <button type="submit" class="act-btn reject" title="إيقاف">
                                                <i class="fas fa-ban"></i> إيقاف
                                            </button>
                                        </form>
                                        <?php endif; ?>
                                        <form method="POST" style="display:inline;"
                                              onsubmit="return confirm('تحذير: سيتم حذف الموظف نهائياً. هل أنت متأكد؟');">
                                            <input type="hidden" name="user_id" value="<?php echo $student['id']; ?>">
...
<head>
    ...
    <title>إدارة موظفي الأرشيف - لوحة تحكم المشرف</title>
...
<body>
    ...
    <div class="header-wrapper">
        <header class="fixed-header">
            <div class="header-content">
                ...
                <p>إدارة موظفي الأرشيف - لوحة تحكم المشرف</p>
            </div>
        </header>
    ...
    <div class="container">
        ...
        <div class="page-hero">
            <div>
                <h2><i class="fas fa-user-tie"></i> إدارة موظفي الأرشيف</h2>
                <p>عرض وإدارة موظفي الأرشيف المسجلين في النظام</p>
            </div>
        ...
        <form method="GET" action="">
            <div class="filter-bar">
                <div class="filter-group">
                    ...
                    <input type="text" name="search" placeholder="اسم الموظف أو البريد أو المعرف..."
                ...
                <div class="filter-group">
                    ...
                    <select name="status">
                        ...
                        <option value="rejected"<?php echo $status_filter === 'rejected' ? 'selected' : ''; ?>>موقوف</option>
                    </select>
                </div>
                <button type="submit" class="filter-btn filter-btn-primary">
                    <i class="fas fa-search"></i> بحث
                </button>
                <a href="manage_archive.php" class="filter-btn filter-btn-reset">
        ...
        <div id="table-area">
            ...
            <div class="students-table-wrap">
                ...
                <div class="empty-state">
                    <i class="fas fa-users-slash"></i>
                    <p>لا يوجد موظفو أرشيف مطابقون للبحث الحالي.</p>
                </div>
                <?php else: ?>
                <div style="overflow-x: auto;">
                    <table class="students-table">
        ...
        <?php
    $footer_about_title = "إدارة موظفي الأرشيف";
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            function updateContent() {
                ...
                fetch('manage_archive.php?' + params.toString())
